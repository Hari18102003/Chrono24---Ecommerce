...
const Womenspage = () => {
    ...
    const womensWatches = watches.filter(watch => watch.gender.toString() === "Women");

    useEffect(() => {
        if (womensWatches.length > 0) {
            const filters = womensWatches.filter(watch => watch.name.toLowerCase().includes(searchTerm.toLowerCase()));
            setSearchedWatches(filters);
        }
        if (searchedWatches.length < 0) {
            setSearchedWatches(womensWatches);
        }
    }, [searchTerm, watches]);
    ...
    const handleFilter = (event) => {
        ...
        if (selectedOption === "below100") {
            const b100filter = womensWatches?.filter(watch => parseFloat(watch.price) < 100.0);
            ...
            const b100filter = womensWatches?.filter(watch => parseFloat(watch.price) > 100.0 && parseFloat(watch.price) < 300.0);
            if (b100filter.length > 0) {
                setSearchedWatches(b100filter);
            }
            else {
            ...
            const b100filter = womensWatches?.filter(watch => parseFloat(watch.price) > 300.0);
            if (b100filter.length > 0) {
                setSearchedWatches(b100filter);
            }
            else {
    ...
    return (
        <div className='mt-4 px-5'>
            <div className='bg-white p-2'>
                <h1 className='text-center font-bold text-xl my-3'>For Womens</h1>
                <div className='md:flex md:items-center md:justify-between mb-3'>
                    <input type='search' placeholder='Search' value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className='border-2 mb-2 md:mb-0 p-2 shadow-md' />
